...
fn mode_instructions(mode: &str) -> &'static str {
    match mode {
        ...
        "problem" => "入力は問題文です。問題文としてLaTeXへ転記してください。",
        "problem_with_subquestions" => "入力は小問付きの問題文です。小問は原文の番号付けを保ちながらenumerate環境へ変換してください。",
...
pub fn output_schema() -> Value {
    json!({
        ...
        "required": ["schemaVersion", "detectedType", "latex", "plainText", "requiredPackages", "warnings", "uncertainFragments", "segments", "suggestedInsertTarget"],
        "properties": {
...
fn run_job(state: &Arc<AppState>, job_id: i64, cancel: &AtomicBool) -> Result<(), String> {
    ...
    prompt.push('\n');
    if opt_bool(&job.options, "faithful", true) && !opt_bool(&job.options, "reformat", false) {
        prompt.push_str("原文に忠実に転記してください。\n");
    }
    if opt_bool(&job.options, "reformat", false) {
        prompt.push_str("文意を変えない範囲で、教材向けに体裁（改行・スペース）を整えてください。\n");
    }
    if opt_bool(&job.options, "enumerateSubquestions", false) {
    ...
    let req = ConversionRequest {
        ...
        developer_instructions: FIXED_INSTRUCTIONS.to_string(),
        prompt_text: prompt.clone(),
...
pub fn update_job_latex(state: &Arc<AppState>, job_id: i64, latex: String) -> Result<(), String> {
    ...
    let conn = state.conn.lock().map_err(err_str)?;
    let (status, current_latex, structured): (String, String, String) = conn
        .query_row(
            "SELECT status, output_latex, structured_result_json
    ...
    result
        .warnings
        .retain(|warning| warning.code != "SUSPICIOUS_COMMAND" && warning.code != "UNSAFE_IMAGE_PATH");
